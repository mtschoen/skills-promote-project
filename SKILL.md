...
- `~/.claude/notes/idioms_linters.md` — **per-language tool selection, the pinned `aislop` version, and gotchas** (e.g. the Python "→ 100" playbook).
...
5. **Register in projdash now** — don't assume auto-discovery (see Registration below).
6. Initial commit.

**Roadmap file:** `project-conventions.md` calls for `.plan` (terse bullets). If adopting a `capture-idea` folder it will already have a `PLAN.md` — **do not clobber it**: projdash's task detection reads `PLAN.md`/`TODO.md`, not `.plan`. Keep the existing `PLAN.md` for tasks; add a `.plan` only if the user wants the terse-roadmap form too. Surface the choice rather than silently renaming.

### Tier 2 — stack-aware (after the stack is fixed)

Apply the literal shapes from `cross-project-config.md`, with tool selection + gotchas from `idioms_linters.md`, tailored to the decided stack:
...
7. Stack-specific must-haves `idioms_linters.md` calls out — e.g. Python: a `requirements.txt` mirroring deps (with a one-line note that `pyproject.toml` is the install source of truth), so `aislop`'s security engine can audit them.
...
- **projdash registration is mandatory, explicit, and early** (Tier 1 step 5):
  - Fresh empty project → projdash MCP `create_project`.
  - **Adopting an existing folder** (the common capture-idea case — `create_project` refuses existing folders) → projdash MCP **`register_existing_path`** (params: `path`, optional `name`/`description`/`status`). Fallback if that tool is absent: the CLI `projdash project register <path>`. Do NOT call `create_project` on an existing folder — it raises `FileExistsError`.
  - Verify it registered (`list_projects`/`get_project`) — don't assume `projdash scan` will find it (the CLI scan ignores `manual_include` paths).
...
- [ ] **Registered in projdash** (verify it appears, don't assume).
...
| Skipping registration, assuming `projdash scan` finds it | Register explicitly in Tier 1 — scan misses `manual_include` paths |
